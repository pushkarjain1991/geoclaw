...
!BOP
!
! !ROUTINE: PDAF_lnetf_alloc --- PDAF-internal initialization of LNETF
!
! !INTERFACE:
SUBROUTINE PDAF_lnetf_alloc(subtype, outflag)

! !DESCRIPTION:
! Perform allocation of arrays for localized NETF.
!
...
! ******************************
  ...
  on_filterpe: IF (filterpe) THEN
    ...
    ALLOCATE(state(dim_p), stat = allocstat)
     ...
     IF (screen > 2) WRITE (*,*) 'PDAF: lnetf_alloc - allocate eofV of size ', &
