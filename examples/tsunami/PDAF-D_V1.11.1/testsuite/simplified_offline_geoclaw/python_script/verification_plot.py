...
def verify_das(dtobs, ens_number, output_times, num_ens):
    ...
    for k,j in enumerate(dtobs[:-1]):
        ...
        read_verification_output = "./ens_"+str(ens_number)+"_"+str(k)+"/fort.q00" + str(output_times)
        test_case = ramr.ReadAmrForLevel(read_verification_output, 1.0)

        # Set the original geoclaw
        original_fortq_time = str((np.size(dtobs)-1)*output_times*int(dtobs[k+1])/int(dtobs[-1])).zfill(2)
...
#        error_reshaped_masked_eta_land, error_reshaped_masked_eta_water, error_reshaped_masked_eta_water_percent = errg.error_geoclaw(test_case, original_case)
        ...
        plotmap.class_contour(error_percent_class, "WSE error %: ens_number = "+str(ens_number)+"; num_ens = " + str(num_ens)+"; time = " + str(dtobs[k+1]), -200.0,200.0)
