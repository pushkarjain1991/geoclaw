...
subroutine flagregions2_assim(mx,my,mbuff,xlower,ylower,dx,dy,level,t, &
    ...
    rloop: do m=1,num_regions_assim
        !if (t < regions_assim(m)%t_low .or. t > regions_assim(m)%t_hi) then
        !    cycle rloop  ! no intersection
        !endif
        
        if (xlower >= regions_assim(m)%x_hi .or. xupper <= regions_assim(m)%x_low) then
            cycle rloop  ! no intersection
        else
            i1 = max(floor((regions_assim(m)%x_low - xlower) / dx) + 1, 1)
            i2 = min(floor((regions_assim(m)%x_hi -xlower) / dx) + 1, mx)
        endif

        if (ylower >= regions_assim(m)%y_hi .or. yupper <= regions_assim(m)%y_low) then
            cycle rloop  ! no intersection
        else
            j1 = max(floor((regions_assim(m)%y_low - ylower) / dy) + 1, 1)
            j2 = min(floor((regions_assim(m)%y_hi - ylower) / dy) + 1, my)
        endif

        do j=j1,j2
            do i=i1,i2
                minlevel(i,j) = max(minlevel(i,j), regions_assim(m)%min_level)
                maxlevel(i,j) = max(maxlevel(i,j), regions_assim(m)%max_level)
            enddo
         enddo
    enddo rloop
...
end subroutine flagregions2_assim
