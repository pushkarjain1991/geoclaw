!$Id: init_dim_obs_l_pdaf.F90 1411 2013-09-25 14:04:41Z lnerger $
!BOP
!
...
! !USES:
  USE mod_assimilation, &
       ONLY: local_range, coords_obs, coords_l, obs_index_l
  USE mod_model, &
       ONLY: nx, ny, nx_p
...
! *** local variables ***
  ...
  INTEGER :: offset                   ! Process-local offset in global state vector
...
! **********************************************
  ...
  END DO
  coords_l(1) = ceiling(real(domain_p+offset)/real(ny))
  coords_l(2) = domain_p + offset - (coords_l(1)-1)*ny

  !Determine coordinate limits for observation domain
  limits_x(1) = coords_l(1) - INT(local_range)
  if (limits_x(1) < 1) limits_x(1) = 1
  limits_x(2) = coords_l(1) + INT(local_range)
  if (limits_x(2) > nx) limits_x(2) = nx

  limits_y(1) = coords_l(2) - INT(local_range)
  if (limits_y(1) < 1) limits_y(1) = 1
  limits_y(2) = coords_l(2) + INT(local_range)
